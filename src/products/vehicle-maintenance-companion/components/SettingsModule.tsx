...
import { detectPushCapability, sendTestPush, subscribeToPush, type PushCapability } from "@/lib/notifications/pushClient";
...
export default function SettingsModule() {
  const { status, errorMessage, instanceId } = useVehicleMaintenance();
  const [prefs, setPrefs] = useState<ReminderPreferences>({ ...DEFAULT_REMINDER_PREFERENCES, timezone: deviceTimezone() });
  ...
  async function changeReminders(on: boolean) {
    ...
    if (await save({ ...prefs, remindersEnabled: true, timezone: deviceTimezone() })) {
      setMessage("Reminders are on. You will hear about jobs that reach their interval, and dates that are close.");
    }
  }
