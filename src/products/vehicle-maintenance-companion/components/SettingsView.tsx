...
import { ScreenHeading } from "./Workshop";
import Alert from "@/design-system/Alert";
import Button from "@/design-system/Button";
import Select from "@/design-system/Select";
import Toggle from "@/design-system/Toggle";
import type { PushCapability } from "@/lib/notifications/pushClient";
import type { ReminderPreferences } from "@/lib/notifications/reminderClock";
...
export default function SettingsView({ prefs, capability, busy, message, onRemindersChange, onDetailChange, onQuietChange, onTest }: SettingsViewProps) {
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
      ...
      <section className="flex flex-col gap-5 rounded-[var(--radius)] border border-[var(--border-strong)] bg-[var(--surface)] p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <Toggle checked={prefs.remindersEnabled} onChange={onRemindersChange} label="Remind me" disabled={busy || (blocked && !prefs.remindersEnabled)} />
        </div>

        {capability === "denied" && (
          <Alert tone="warning">Notifications are blocked in this browser&apos;s site settings. Allow them there, reload, and this will switch on.</Alert>
        )}
        {capability === "unsupported" && (
          <Alert tone="info">This browser cannot receive notifications, so reminders are not available here. Everything else works.</Alert>
        )}
        ...
        {prefs.remindersEnabled && (
          <>
            <div className="flex items-start justify-between gap-4 border-t border-[var(--border)] pt-5">
              ...
              <Toggle checked={prefs.showDetail} onChange={onDetailChange} label="Say what it is about" disabled={busy} />
            </div>
            ...
            <div className="border-t border-[var(--border)] pt-5">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                ...
                <Select label="Until" value={prefs.quietEndHour} disabled={busy} onChange={(event) => onQuietChange(prefs.quietStartHour, Number(event.target.value))}>
                  {HOURS.map((hour) => (
                    <option key={hour} value={hour}>
                      {hourLabel(hour)}
                    </option>
