...
export async function loadReminderPreferences(instanceId: string): Promise<Result<ReminderPreferences>> {
  const { data, error } = await supabase.from("vmc_notification_preferences").select(COLUMNS).eq("product_instance_id", instanceId).maybeSingle();
...
export async function saveReminderPreferences(instanceId: string, preferences: ReminderPreferences): Promise<Result<ReminderPreferences>> {
  const {
    data: { session },
    error: sessionError,
