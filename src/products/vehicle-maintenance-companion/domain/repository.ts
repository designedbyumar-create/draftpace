...
/**
 * The shared CRUD implementation both this product's domain modules
 * (vehicles.ts, maintenanceItems.ts) are built from. This product's own
 * copy of the same pattern Home Base and Personal Finance Companion each
 * keep, deliberately not imported across products.
 *
 * Ownership is enforced by RLS + the `_vmc_owns_instance` check in the
