...
/**
 * Instance resolution for In Order. Same "continuous" shape as Home Base
 * and PFC: one instance ever, found by slug and most recently created,
 * ignoring cycle_key.
 *
...
export const PERSONAL_LIFE_AFFAIRS_COMPANION_SLUG = "personal-life-affairs-companion";

export async function findInOrderInstanceId(): Promise<FindInstanceResult> {
  const { data, error } = await supabase
    .from("product_instances")
    .select("id")
    .eq("product_slug", PERSONAL_LIFE_AFFAIRS_COMPANION_SLUG)
