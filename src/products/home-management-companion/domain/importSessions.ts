...
/**
 * Creates the one row per paste/upload that every record created from it
 * references via import_session_id, Home Base's own parallel to PFC's
 * domain/importSessions.ts. Raw file bytes are never persisted here, only
