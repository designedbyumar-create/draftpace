    ...
    >
      ...
      <div className="w-full max-w-sm bg-white border border-gray-200 rounded-3xl shadow-sm overflow-hidden">
        ...
        <div className="p-6 sm:p-8">
          ...
          {/* Title */}
          <h1 className="text-[22px] sm:text-[26px] font-black text-gray-950 mb-1 leading-tight"
          ...
          {step === 1 && (
            <div className="mt-5">
              ...
              <input
                ...
                autoFocus
                className="w-full h-[50px] bg-gray-50 border border-gray-200 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-100 rounded-xl px-4 text-[15px] text-gray-900 placeholder-gray-400 transition-all mb-4"
              />

              <button onClick={() => email && setStep(2)} disabled={!email}
                className="w-full h-[50px] bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-[15px] mb-4">
          ...
          {step === 2 && (
            <div className="mt-5">
              ...
              <label className="block text-[12px] font-semibold text-gray-600 mb-2">Password</label>
              <div className="relative mb-2">
                <input
                  ...
                  autoFocus
                  className="w-full h-[50px] bg-gray-50 border border-gray-200 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-100 rounded-xl px-4 pr-14 text-[15px] text-gray-900 placeholder-gray-400 transition-all"
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-[11px] font-semibold text-gray-400 hover:text-gray-600">
                  ...
                  <div className="flex gap-1 flex-1">
                    {[1,2,3].map(i => (
                      <div key={i} className="h-1 flex-1 rounded-full transition-all"
                        style={{ background: password.length >= i * 4
                          ? i===1 ? "#f97316" : i===2 ? "#f59e0b" : "#22c55e"
                          : "#e5e7eb" }}/>
                    ))}
                  </div>
              ...
              <button onClick={() => password.length >= 8 && setStep(3)} disabled={password.length < 8}
                className="w-full h-[50px] bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-[15px] mb-3">
                Continue <ArrowRight size={14}/>
              </button>
          ...
          {step === 3 && (
            <div className="mt-5">
              <p className="text-[13px] text-gray-400 mb-5">Optional — we'll use your email name if you skip.</p>

              <label className="block text-[12px] font-semibold text-gray-600 mb-2">Your name</label>
              <input
                ...
                autoFocus
                className="w-full h-[50px] bg-gray-50 border border-gray-200 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-100 rounded-xl px-4 text-[15px] text-gray-900 placeholder-gray-400 transition-all mb-5"
              />
              ...
              <button onClick={handleSignup} disabled={loading}
                className="w-full h-[50px] bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white font-bold rounded-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-[15px] mb-3">
                {loading ? (
