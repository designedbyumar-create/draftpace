...
const CONFIG = {
  STATS_READY:    false,
  PLANNERS_READY: false,
} as const;

const UPCOMING = [
  { id:"1", title:"Monthly Budget Reset",  type:"Planner",   description:"Track every dollar. Reset every month. Take back control of your finances.", duration:"30 days",  barColor:"#4f46e5", typeColor:"#4338ca", typeBg:"#eef2ff", typeBorder:"#c7d2fe", iconPath:"M3 3h18v18H3z M9 9h6M9 12h6M9 15h4",                                          accentFrom:"#e0e7ff", accentTo:"#c7d2fe" },
  { id:"2", title:"30-Day Habit Builder",  type:"Checklist", description:"Build any habit in 30 days with daily check-ins and streak tracking.",        duration:"30 days",  barColor:"#7c3aed", typeColor:"#6d28d9", typeBg:"#fdf4ff", typeBorder:"#ddd6fe", iconPath:"M9 11l3 3L22 4 M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11", accentFrom:"#ede9fe", accentTo:"#ddd6fe" },
  { id:"3", title:"Savings Goal — $5,000", type:"Guide",     description:"Weekly contributions. Visual progress. Hit any savings target on schedule.",  duration:"12 weeks", barColor:"#0d9488", typeColor:"#0f766e", typeBg:"#f0fdfa", typeBorder:"#99f6e4", iconPath:"M12 1v22M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6",                accentFrom:"#ccfbf1", accentTo:"#99f6e4" },
];
...
function QuoteCard() {
  ...
  return (
    <div className="mx-2 mb-2 rounded-2xl overflow-hidden relative" style={{ height:"96px" }}>
      <svg width="100%" height="96" viewBox="0 0 200 96" preserveAspectRatio="xMidYMid slice" className="absolute inset-0">
        <defs>
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-3">
        <p className="text-[11px] font-medium leading-[1.4] mb-0.5" style={{ color:"rgba(255,255,255,0.9)" }}>
          {quote.text} ✨
        </p>
      ...
      <div className="px-2 py-3 flex-1 overflow-y-auto">
        ...
        {UPCOMING.map((item) => (
          <div key={item.id} className="flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-[12px] font-medium text-gray-400 cursor-default">
            <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: CONFIG.PLANNERS_READY ? item.barColor : "#d1d5db" }}/>
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-baseline gap-2 mb-1">
            <span className="font-black leading-none tracking-tight" style={{ fontSize:"clamp(44px,8vw,60px)", color:"#ea580c" }}>
              {stats.streak}
            </span>
          ...
          <p className="text-[13px] text-gray-400 max-w-xs">{getStreakMessage(stats.streak, stats.personalBest)}</p>
        </div>
...
// ── FIXED: plain divs instead of Skeleton for dynamic heights ─────────────────
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-end gap-1.5 shrink-0">
          {[32,20,28,32,24,12,10].map((h, i) => (
            <div key={i} className="flex flex-col items-center gap-1">
              <div
                className="w-3 rounded-sm animate-pulse bg-gray-200"
                style={{ height:`${h}px` }}
              />
              <div className="w-2 h-2 rounded animate-pulse bg-gray-200"/>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-5 lg:p-6">
        ...
        <p className="text-[13px] text-gray-400 italic mb-5 leading-relaxed">
          {planner.context.split(planner.contextHighlight).map((part, i, arr) =>
            i < arr.length - 1 ? (
              <span key={i}>{part}<strong className="not-italic font-semibold text-gray-700">{planner.contextHighlight}</strong></span>
            ) : part
          )}
        </p>
        <div className="mb-5">
          <div className="flex justify-between items-baseline mb-2">
            ...
            <span className="text-[28px] font-black leading-none tracking-tight" style={{ color: planner.barColor }}>
              {planner.progress}%
            </span>
          </div>
      ...
      <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden relative">
        ...
        <div className="absolute right-0 top-0 w-48 h-36 pointer-events-none overflow-hidden">
          <svg viewBox="0 0 200 150" fill="none" className="absolute inset-0 opacity-[0.07]">
            ...
            <path d="M0 150 L60 80 L100 110 L140 50 L180 80 L200 60 L200 150Z" fill={featured.barColor} opacity=".6"/>
            <path d="M0 150 L80 90 L120 120 L160 70 L200 90 L200 150Z" fill={featured.barColor} opacity=".5"/>
          </svg>
        </div>
        <div className="relative p-5 lg:p-6">
          ...
          <button disabled className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-[14px] font-bold text-gray-400 bg-gray-100 cursor-not-allowed">
            <Icon path={ICONS.clock} size={13} color="#9ca3af"/> Coming soon
          </button>
        </div>
      </div>
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      {[
        ...
        <div key={s.label} className="rounded-2xl px-4 py-3 border text-center"
          style={{ background:s.bg, borderColor:s.border }}>
          <p className="text-[20px] font-black leading-none mb-0.5 tracking-tight" style={{ color:s.color }}>{s.val}</p>
...
// ── Paused Nudge ──────────────────────────────────────────────────────────────
function PausedNudgeCard({ planner }: { planner: PausedNudge }) {
  return (
    <div className="bg-white border border-amber-200 rounded-2xl p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-gray-400">Paused {planner.daysAgo} days ago · {planner.progress}% done</p>
        <div className="h-1.5 bg-amber-100 rounded-full mt-2 overflow-hidden">
          <div className="h-full bg-amber-400 rounded-full" style={{ width:`${planner.progress}%` }}/>
        </div>
      </div>
      <Link href={`/dashboard/planner/${planner.id}`}
...
export default function DashboardPage() {
  ...
  const mockFocus: ActivePlanner = {
    ...
    progress:68, barColor:"#4f46e5", meta:"Day 14 of 30",
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 lg:px-8 py-5 lg:py-7 max-w-3xl mx-auto w-full space-y-4 lg:space-y-5 pb-24 lg:pb-10">
          ...
          <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4, delay:0.05 }}>
            {CONFIG.STATS_READY ? <StreakHero stats={mockStats}/> : <SkeletonStreakHero/>}
          </motion.div>
          ...
          <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4, delay:0.1 }}>
            <div className="flex items-center justify-between mb-3">
              ...
              {!CONFIG.PLANNERS_READY && (
                <Link href="/dashboard/explore" className="text-[12px] font-medium text-indigo-600 hover:underline flex items-center gap-1">
                  Browse planners <Icon path={ICONS.arrow} size={11} color="#4f46e5"/>
                </Link>
              )}
            </div>
            {CONFIG.PLANNERS_READY ? <TodaysFocusCard planner={mockFocus}/> : <TodaysFocusTeaser/>}
          </motion.div>

          <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4, delay:0.15 }}>
            {CONFIG.STATS_READY ? <QuickStatsRow stats={mockStats}/> : <SkeletonQuickStats/>}
          </motion.div>
          ...
          <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4, delay:0.2 }}>
            <div className="relative rounded-2xl overflow-hidden px-5 lg:px-6 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              ...
              <div className="relative">
                <p className="text-[10px] font-bold uppercase tracking-widest text-indigo-500 mb-1">Unlock everything</p>
